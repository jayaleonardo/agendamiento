...
describe('Contacto Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./contacto-detail.component').then(m => m.ContactoDetailComponent),
              resolve: { contacto: () => of({ id: 123 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('Should load contacto on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', ContactoDetailComponent);

      // THEN
      expect(instance.contacto()).toEqual(expect.objectContaining({ id: 123 }));
    });
  });
